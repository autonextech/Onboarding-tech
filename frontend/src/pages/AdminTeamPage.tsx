...
import { Plus, Trash2, Mail, Globe, MessageCircle, Building2 } from 'lucide-react';
...
export default function AdminTeamPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ name: '', role: '', department: '', email: '', linkedin: '', slack: '' });
        setShowForm(false);
        fetchTeam();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-primary hover:bg-primary-container text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 shadow-sm">
          <Plus className="h-4 w-4" /> Add Team Member
        </button>
      </div>

      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
          <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-100 pb-3">New Team Member Profile</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">LinkedIn Profile Name/URL</label>
              <input type="text" value={formData.linkedin} onChange={e => setFormData({...formData, linkedin: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary outline-none text-sm" placeholder="e.g. /in/emilyrodriguez" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Slack Handle / Channel</label>
              <input type="text" value={formData.slack} onChange={e => setFormData({...formData, slack: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary outline-none text-sm" placeholder="e.g. @emily_hr" />
            </div>
            <div className="md:col-span-2 flex justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
              ...
              <button type="submit" disabled={submitting} className="bg-primary hover:bg-primary-container text-white px-6 py-2 rounded-lg text-sm font-bold transition-all disabled:opacity-50">
                {submitting ? 'Saving...' : 'Add Member to Directory'}
              </button>
            </div>
          </form>
        </motion.div>
            ...
            {team.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-12 text-center text-slate-500">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Add colleagues to the directory so candidates can meet them.</p>
                </td>
              </tr>
            ) : (
              team.map(member => (
                <tr key={member.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 space-y-1">
                    {member.email && (
                      <div className="flex items-center gap-2 text-xs text-slate-600 hover:text-primary cursor-pointer transition-colors">
                        <Mail className="h-3 w-3" /> {member.email}
                      </div>
                    )}
                            {member.linkedin && (
                              <div className="flex items-center gap-2 text-xs text-slate-600 hover:text-primary cursor-pointer transition-colors">
                                <Globe className="h-3 w-3" /> {member.linkedin}
                              </div>
                            )}
                            {member.slack && (
                              <div className="flex items-center gap-2 text-xs text-slate-600 hover:text-primary cursor-pointer transition-colors">
                                <MessageCircle className="h-3 w-3" /> {member.slack}
                              </div>
                            )}
                    {!member.email && !member.linkedin && !member.slack && <span className="text-xs text-slate-400 italic">No contact info</span>}
                  </td>
                  <td className="py-4 px-6 text-right">
                    <button onClick={() => handleDelete(member.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
