...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        for (const p of allProgress) {
            if (!progressByUser.has(p.userId))
                progressByUser.set(p.userId, new Map());
        ...
        for (const a of allQuizAttempts) {
            if (!quizByUser.has(a.userId))
                quizByUser.set(a.userId, new Map());
        ...
        const activeCandidates = allCandidates.filter((u) => u.isActive);
        const result = {
            metrics: {
                totalCandidates: candidates.length,
                activeCandidates: activeCandidates.length,
                inactiveCandidates: allCandidates.length - activeCandidates.length,
                completedCandidates: enrichedCandidates.filter((c) => c.status === 'completed').length,
                avgProgress,
