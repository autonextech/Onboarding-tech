...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const [totalCandidates, totalModules, recentCandidates, progressData] = await Promise.all([
            ...
            prisma_1.default.module.count({ where: { status: { in: ['published', 'PUBLISHED', 'Published'] } } }),
            ...
            prisma_1.default.candidateProgress.groupBy({
                by: ['userId'],
        ...
        if (progressData.length > 0) {
            const totalCompleted = progressData.reduce((acc, p) => acc + p._count.sectionId, 0);
...
router.get('/full', async (req, res) => {
    try {
        ...
        candidates.forEach((candidate) => {
            ...
            if (cProgress <= 25)
                distribution['0-25%']++;
            else if (cProgress <= 50)
                distribution['26-50%']++;
            else if (cProgress <= 75)
                distribution['51-75%']++;
            else
                distribution['76-100%']++;
            if (cProgress === 100) {
